...
- 적 처치 시 골드 획득
- 골드로 영웅 스탯 업그레이드 (최대 레벨 = 캐릭터 레벨)
- **1**: 공격력 +3, **2**: 속도 +0.08, **3**: HP +25, **4**: 추가골드 +2
...
### V1.14.2 (현재)
- **경험치 중복 저장 버그 수정**
  - 게임 종료 시 경험치가 여러 번 저장되어 레벨이 급격히 오르던 문제 해결
  - useEffect 의존성 배열 문제로 인한 중복 호출 방지
- **싱글/멀티플레이 로직 동기화**
  - 맵 크기 통일 (3000×2000)
