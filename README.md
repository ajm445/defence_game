...
> 🏆 RPG 로비에서 **랭킹** 버튼을 클릭하면 난이도별 클리어 기록을 확인할 수 있습니다.

- **난이도 탭**: 극한(빨강) / 지옥(주황) / 종말(보라) 3개 난이도 독립 랭킹
- **인원수 탭**: 1인/2인/3인/4인 별도 랭킹
- **표시 정보**: 순위, 클리어 시간, 참여 플레이어 정보
- **플레이어 정보**: 닉네임, 직업(전직 포함), 캐릭터 레벨
- **상위 10개**: 각 난이도/인원수별 빠른 클리어 순으로 정렬
- **메달 표시**: 1위 🥇, 2위 🥈, 3위 🥉
...
- 개인 메시지(DM): 온라인 친구에게 1:1 채팅 (서버 메모리 전용, 비속어 필터 적용)
...
- 게임 종료 후 준비 시스템: 비호스트 10초 내 준비 필수, 미준비 시 자동 퇴장
- 방장 시작 타이머: 모든 플레이어 준비 후 30초 내 미시작 시 방 파기
- 호스트 위임: 방장 퇴장 시 남은 플레이어에게 자동 권한 이전 (방 유지)
- 초대 시스템: 친구 탭에서만 초대 가능, 같은 방 친구에게는 초대 불가
...
| 인증 API (로그인/회원가입) | IP당 분당 20회 (429) |
...
- **닉네임 서버 검증**: 길이(2~10자), 형식(한글/영문/숫자/일부 특수문자), 비속어 필터
...
### V1.26.8 (현재)
- **보안 강화**: REST API JWT 인증, XSS 방지(sanitizeText), 입력 검증, 랭킹 조작 방지, WebSocket 레이트 리밋
- **재접속 시스템**: 60초 유예 + 수동 재접속 UI + 영웅 유지 + 미복귀 시 EXP 0 패널티
- **레인저 W 다중 화살 보정**: 첫 화살 100%, 이후 50% 감소형 데미지 (서버/클라이언트 동기화)
- **데이터 동기화**: 적 스탯/부활 HP/마법사 패시브 서버-클라이언트 통일
- **버그 수정 (15건+)**: React 훅 의존성, 쿨다운 비동기, stale closure, 이펙트 정리, 0 나누기 등
- **UX 개선**: 줌아웃 범위 축소(0.75), 기지 수 동적 표시, 호스트 변경 후 readyCheck 리셋

### V1.26.7
- **서버-클라이언트 스탯/설정 동기화**: 기본 4직업 스탯 통일, shockwave 즉사 데미지 통일, 스킬 쿨다운 공유 설정 통합
- **버그 수정**: mage_e 보스 데미지 배율 적용, healing_light 이펙트 위치 추적, 비호스트 경험치 처리 안전성
- **UX 개선**: 보스 페이즈 진입 사운드+화면 플래시, 다크나이트 E 토글 툴팁, 난이도 선택 로비 복귀 시 유지
- **도감 개선**: 전직 직업 해금 표시 (자물쇠+"전직 필요"), 기본 직업 "플레이어 Lv." 명시
- **직업 기억 개선**: 계정별 localStorage 분리, 게스트는 궁수 고정
- **죽은 코드 정리**: RPGClassSelectScreen 제거, skillCooldownReduction 직렬화 추가
- **기타**: 로비 채팅 100개 제한, attackSpeed 주석 명확화, 스킬 툴팁 "기본 쿨타임" 표시

### V1.26.6
- **아크메이지/힐러 스프라이트 완성**: tier1/tier2 각 8종 (걷기/공격/W/E) + 3번 프레임 pendingSkill 싱크
- **기지 공격 프레임 싱크 통합**: 다크나이트/아크메이지/힐러 기지 공격도 executeSkill 경유 (적↔기지 전환 시 타이밍 일관성)
- **메테오 샤워 버그 수정**: 원형 랜덤 분포 + 중심점 보존 + 고정 interval + 이펙트 중복 제거
- **힐러 이펙트 버그 수정**: healPercent 분기 충돌, 생명의 샘 중복 이펙트, 0 데미지 표시, 멀티 핸들러 누락
- **마지막 선택 직업 기억**: localStorage 기반 직업 저장/복원 (로그아웃/재로그인 유지)
- **스프라이트 프롬프트 수정**: 아크메이지/힐러 이펙트 묘사 제거, 캐스팅 포즈만 + NOTE 추가

### V1.26.5
- **다크나이트 스프라이트 완성**: tier1/tier2 전체 8종 + 기본공격 pendingSkill 프레임 싱크 (데미지/이펙트/사운드 동일 틱)
- **다크나이트 E 토글 모션**: ON 시 E 스프라이트 재생 + 보라색 불길 오라 렌더링, OFF 시 모션 없음
- **다크나이트 W/E 폴백 버그 수정**: HP 부족 시 기사 기본 스킬 실행 방지 + 스킬 버튼 비활성화
- **기본 직업 쿨다운 통일**: 전사 W(7초), 궁수 W(8초)/E(30초), 기사 W(8초), 마법사 W(7초) 서버-클라 일치

### V1.26.4
- **팔라딘 스프라이트 완성**: tier1/tier2 걷기/공격/W/E 8종 + W/E frameTimes 싱크 + E스킬 3번째 프레임 pendingSkill 발동
- **다크나이트 스프라이트 추가**: tier1 걷기/공격 2종 + 공격 프레임 가중치 싱크 (ATTACK_FRAME_WEIGHTS)
- **레인저 스프라이트 추가**: tier1/tier2 7종 + W스킬 3번째 프레임 pendingSkill + E스킬 회오리 이펙트
- **전사/버서커 쿨다운 변경**: 지속형 버프 종료 후 쿨다운 시작 (전사 30초/버서커 45초 실질 대기)
- **프롬프트 변경**: 팔라딘 E(방패 들어올리기), 다크나이트 W(보라색 에너지 충전)

### V1.26.3
- **궁수 계열 패시브 리워크**: 궁수 다중타겟 2→3명, 저격수 전직 시 다중타겟→공격력 증가 전환, 레인저 전직 시 최대 5명 강화
- **팔라딘 힐 기준 변경**: 기본공격 힐 아군HP 5%→자신HP 2%, 궁극기 아군HP 30%→자신HP 20% 전체 회복
- **적 기지 균열 이펙트**: HP 감소에 따라 성벽에 균열이 점진적으로 추가 (80%~0% 4단계 + 파편)
- **직업 도감 패시브 강화**: 전직별 기본 패시브 변화 안내 (버서커/저격수/레인저/대마법사), 팔라딘 힐 설명 수정
- **저격수 신속 버프 이펙트**: W스킬 사용 후 3초간 시안색 공전 파티클 + 글로우 (swiftness 버프 시각화)
- **보스/레인저 스프라이트 수정**: 보스 기본공격 방향 수정, 레인저 기본공격 단일 화살로 변경

### V1.26.2
- **기본공격 프레임 싱크**: 기사/가디언/버서커 기본공격 3번째 프레임에서 타격 발동 (가중 프레임 분배: 15/15/40/30%)
- **저격수 E 개선**: frameTimes 시스템으로 프레임별 타이밍 직접 지정 + 캐스팅 방향 실시간 갱신
- **가디언 차별화**: 공격 사거리 80→60px + 메이스 전용 파티클 이펙트 (attack_mace)
- **전직 모션 스프라이트 확장**: 버서커(E/W) + 가디언(attack/walk/W/E) + 저격수(W/E) + 레인저 추가
- **모션 타이밍 전면 확장**: 전 전직 W/E 스킬에 개별 holdTime/fps 오버라이드 적용
- **버서커 Tier2 시안 불꽃**: 광전사 버프 tier2 전용 시안 색상 이펙트 (글로우/화염/입자/텍스트)
- **공격 방향 폴백**: attackTarget 없을 때 basicAttackEffects 기반 방향 결정
- **스프라이트 프롬프트 정리**: LEFT 방향 강제, 가디언 메이스 수정, 프리픽스 간결화

### V1.26.1
- **이동 버그 수정**: 시전 후 정지 신호 누락, 레이트 리미터 정지 신호 차단, 서버 500ms 이동 타임아웃 추가
- **기사/마법사 모션 완성**: 기사 4종 + 마법사 4종 스프라이트 추가 (기본 4직업 전체 16종 완성)
- **적 유닛 모션 적용**: 기본 적 유닛에 영웅 스프라이트 공유, 서버에서 state/attackCooldown 직렬화
- **모션-스킬 싱크**: 즉발 스킬 attackLockUntil(기본공격 잠금), W/E 모션 중 Q 감지 차단, 공격 방향 flip 고정
- **스킬 타이밍 동기화**: 궁수 W(0.33초 지연), 궁수 E(0.6초 지연) pendingSkill로 모션과 이펙트 싱크
- **마법사 운석 이펙트 수정**: mage_e→mage_meteor 타입 변환, 폭발 duration 0.5→1.5초
- **캐릭터별 모션 오버라이드**: 기사 W(12fps/0.4초), 전사 E(holdTime 1.2초) 개별 타이밍 조정
- **보스 모션 스프라이트**: 보스 1(기마 기사) walk/attack + 보스 2(어둠 마법사) walk/attack 4종 추가
- **스프라이트 비율 보정**: 높이 기준 비율 유지로 모션 전환 시 캐릭터 크기 일정 유지
- **적 시선 방향 수정**: 타겟팅 시에만 영웅 방향, 비타겟팅 시 넥서스(이동 목표) 방향
- **로그아웃 방 상태 초기화**: 로그아웃 후 재로그인 시 이전 방 복귀 버그 수정

### V1.26.0
- **스프라이트 모션 시스템**: 2×2 그리드 스프라이트 시트 기반 캐릭터 애니메이션 (Walk/Attack/W/E 4모션)
- **모션 감지**: 쿨다운 점프 기반 공격/스킬 사용 감지, 돌진/시전 상태 자동 전환, idle 시 정적 이미지 폴백
- **방향 시스템**: 3단계 flip 모드 (normal/invert/none) — 왼쪽 방향 기본, 방향 무관 모션 고정
- **스킬 이펙트 색상 수정**: W/E 스킬 이펙트 27개에 heroClass 추가 → 직업별 색상 정상 적용
- **쿨다운 UI 수정**: 쉐도우 100%부터 시작, 부드러운 전환
- **에셋**: 전사 4종 + 궁수 4종 모션 스프라이트 (왼쪽 방향 통일)
- **스프라이트 도구**: 프레임 반전(`flip-frames.cjs`), 프레임 합성(`combine-sprites.cjs`)

### V1.25.3
- **네트워크 최적화**: 델타 직렬화(변경된 필드만 전송) + 시각 이펙트 15Hz 분리 스트림 + 좌표/HP 정수 양자화로 대역폭 절감
- **입력 ACK 프로토콜**: 클라이언트 입력 시퀀스 번호 → 서버 ACK 반환 시스템
- **서버 보안 강화**: 글로벌 메시지 속도 제한(초당 120), 이동 속도 검증(속도핵 방지), 스킬 타겟 사거리 검증, DB/소셜/채팅 12종 레이트 리미터
- **좀비 연결 감지**: Ping/Pong 30초 간격, 미응답 시 자동 종료
- **서버 성능**: userId 조회 O(1) 인덱스, 방 목록 브로드캐스트 100ms 디바운싱
- **타입 안전성**: `useNetworkSync`의 `any` → 구체적 타입 교체

### V1.25.2
- **소리 설정 시스템 개편**: 로그인 전 소리 설정 가능 (localStorage), 로그아웃 후에도 설정 유지, 전 화면 소리 설정 버튼 배치
- **메인 메뉴/프로필 재구성**: 설정 모달 → 소리 버튼만 유지, 비밀번호 변경·회원 탈퇴는 프로필 화면으로 이동 (메인 메뉴 접근 시만)
- **RPG 인게임 비주얼 개선**: 원형 베이스 → 타원형 그림자, 레벨 배지 제거, 피격 효과 개선, 체력바 간격 축소, 격자선 숨김
- **저격수 궁극기 리워크**: 보스 전용 타겟 + 경로 피격 시스템 (탄환 경로의 적이 대신 피격)
- **스킬바 아이콘 개선**: 다크나이트 토글 쿨다운 수정, 비활성화 시각 피드백, 토글 ON 보라빛 틴트
- **직업 선택 시스템 개선**: 전직/강화 정보 반영, 실제 스탯 표시, 전직 단계 뱃지, 캐릭터 이미지 적용
- **직업 도감 개선**: 캐릭터 이미지 적용, 전직 카드 스타일 통일, 계열 테마 색상, 전직 경로 탭 분리, 영어 직업명 제거
- **DM 버그 수정**: 내가 보낸 메시지 unread 카운트 제외, 친구 탭 메시지 글로우 알림
- **적 기지 파괴 표시**: 보스 출현 시 DESTROYED 텍스트·잔해 숨김
- **BGM 수정**: RTS 튜토리얼 후 메인 메뉴 BGM 미재생 문제 해결

### V1.25.1
- **크로스 플랫폼 이모지 통일**: 36개 컴포넌트의 유니코드 이모지를 Twemoji SVG 기반 `<Emoji>` 컴포넌트로 일괄 교체
  - OS별로 다르게 보이던 이모지가 모든 환경(Windows, Mac, Linux, iOS, Android)에서 동일하게 렌더링
  - 화면, UI 컴포넌트, 터치 UI 전체 적용

### V1.25.0
- **MP3 BGM 시스템**: Web Audio API 심리스 루프, 자동 무음 트리밍, 개별 볼륨 설정
- **BGM 적용**: 로그인(첫 인터랙션), 메인 메뉴, 클래스 선택, 로비, 인게임 전 화면 BGM 재생
- **배경 이미지**: 메인 메뉴, 로그인, 게임 선택 화면에 일러스트 배경 적용
- **로그인 UX 개선**: 탭 설명 추가, 게스트→"바로 시작" 리브랜딩, 안내 문구 개선
- **UI 텍스트 정리**: 영어 텍스트 한글화, 방 생성 모달 영문명 제거

### V1.24.8
- **관리자 인증 미들웨어 라우터 레벨 적용**: per-route → 라우터 마운트 시점에 `requireAdmin` 일괄 적용, 새 route 추가 시 인증 누락 방지
- **관리자 경로 비공개화**: `VITE_ADMIN_PATH` 환경변수로 관리자 페이지 URL 경로 설정 가능 (기본값: `admin`)
  - 프로덕션에서 추측 불가능한 경로 설정 시 `/admin` 접속으로 관리자 페이지 노출 차단
  - 모든 하드코딩 `/admin/` 경로를 `ADMIN_BASE` 상수로 교체

### V1.24.7
- **에셋 폴더 구조 정리**: `public/img/units/RPG/` → `heroes/`, `map/`, `skill_icon/` 하위 폴더 분리
- **숲 맵 이미지 에셋**: 풀, 돌, 바위, 나무, 웅덩이 5종 이미지 에셋 적용 (캔버스 프리미티브 → 이미지, 폴백 유지)
- **스킬 아이콘 이미지 시스템**: 기본 4직업 + 전직 8직업 총 24개 W/E 스킬 아이콘 이미지 적용
  - 스킬바: 버튼 전체 이미지 + 쿨다운 오버레이(위→아래) + 키 표시 버튼 아래 이동
  - 직업 도감: 스킬 카드에 아이콘 이미지 표시

### V1.24.6
- **RPG 맵 테마 시스템**: 숲(Forest), 얼음(Ice), 화산(Volcano), 그림자(Shadow) 4종 테마
  - 테마별 비주얼: 배경 그라데이션, 격자, 영역 색조, 미니맵, 장식물, 경계, 환경 파티클 전부 변경
  - `MapThemeConfig` 설정 기반 구조로 새 테마 추가 시 config만 작성
- **RPG 맵 UI 대폭 개선**
  - 지형 장식: 풀(150개), 바위(40개), 물웅덩이(12개, 반짝임), 횃불(16개, 불꽃 + 글로우)
  - 맵 경계 자연화: 나무/바위 벽 2열 배치, 경계 밖 그라데이션 어둠(250px 페이드 → 검정)
  - 영역별 색조 + 환경 파티클 애니메이션
  - 시드 기반 랜덤 생성, 테마 변경 시 캐시 자동 재생성
- **맵 테마 멀티플레이 동기화**
  - 방 생성 모달에 테마 선택 UI (이모지 + 테마 색상)
  - 로비에서 방장이 실시간 테마 변경 가능
  - 서버-클라이언트 전 과정 동기화 (생성/설정변경/게임시작/로비복귀)

### V1.24.5
- **글로벌 Error Boundary 추가**: 예기치 않은 렌더링 오류 시 흰 화면 대신 안내 화면 + 새로고침 버튼 표시
- **게임 종료 후 준비 시스템**
  - 비호스트 10초 준비 카운트다운, 미준비 시 자동 퇴장
  - 준비 취소 시 타이머 재시작, 모든 준비 완료 시 방장에게 30초 시작 경고
  - 30초 내 미시작 시 방 자동 파기
- **호스트 위임**: 방장 퇴장 시 방 파기 → 남은 플레이어에게 권한 자동 이전
- **초대 시스템 정리**: 온라인 탭 초대 버튼 제거, 친구 탭에서만 초대 가능
- **RTS 1v1 항복**: 인게임 항복 버튼 + 확인 모달 추가
- **게임 나가기 확인 모달**: PauseScreen에서 나가기/중단 시 확인 모달 표시
- **서버 연결 끊김 알림**: 재연결 실패 시 화면 하단 배너 + 새로고침 버튼
- **ESC 키 모달 닫기**: 모든 주요 모달에 ESC 키 닫기 지원
- **서버 보안 강화**: 인증 API IP Rate Limiting(분당 20회), 닉네임 비속어 필터
- **닉네임 길이 제한**: 2~20자 → 2~10자 변경
- **피드백 EXP 보상**: 피드백 작성 시 +50 EXP (1회)
- **관리자 모니터링**: 서버 상태에 점검 여부 뱃지 표시
- **Google Analytics**: 사용자 행동 분석을 위한 GA4 트래킹 추가

### V1.24.4
- **유저 피드백/별점 시스템**
  - 로그인 유저(게스트 제외) 대상 별점(1~5) + 의견 수집 모달
  - 계정당 1회 작성, 이후 수정 가능 (upsert)
  - 전체 평균 별점 및 참여자 수 표시
  - 관리자 페이지에서 전체 피드백 목록 조회 + 통계 확인
- **서버 점검 모드 시스템**
  - 관리자 페이지에서 점검 활성화 (카운트다운 분 설정)
  - 온라인 플레이어 전원에게 실시간 점검 알림 (WebSocket 브로드캐스트)
  - 카운트다운 종료 시 자동 로그아웃 + BGM 정지
  - 인게임: 5초 자동 소멸 알림, 로비: X 버튼 닫기 토스트
  - 점검 중 로그인 차단, 로그인 화면에 점검 안내 표시
  - 관리자 점검 해제 후 정상 운영 복귀
- **친구 시스템 개선**
  - 친구 목록에서 게임 모드(RPG/RTS) 표시
  - DM 메시지 유실 수정 (경합 조건 해결)
- 메인 메뉴 저작권 표시 추가

### V1.24.3
- **친구 간 개인 메시지(DM) 기능 추가**
  - 친구 목록에서 온라인 친구에게 1:1 채팅 가능 (사이드바 💬 버튼)
  - 서버 메모리 전용 (DB 저장 없음), 비속어 필터 및 스팸 방지(500ms) 적용
  - 안읽은 메시지 빨간 배지 표시, 친구 오프라인 시 대화 자동 정리
  - 신규 파일: `DirectMessageManager.ts`(서버), `DMChatWindow.tsx`(클라이언트)
- **친구 시스템 안정성 개선**
  - 친구 요청/수락 시 중복 표시 수정 (`addFriend`, `addPendingRequest` 중복 체크 추가)
  - 친구 삭제 시 즉시 UI 반영 (폴링 쿨다운 메커니즘으로 낙관적 업데이트 보호)
  - 폴링 간격 5초 → 30초로 변경 (실시간 이벤트가 주 업데이트 경로)
  - 서버 수락 응답 시 `FRIENDS_LIST` 중복 전송 제거
  - `useFriendMessages` 핸들러 다중 등록으로 인한 메시지 중복 처리 수정
  - 친구 요청 알림 모달(`FriendRequestNotification`) 제거 (사이드바에서 처리)

### V1.24.2
- **Boss2 암흑 유성 낙하 이펙트**: dark_meteor 스킬에 보라색 운석 낙하 시각 이펙트 추가
  - 5개 운석 시간차 낙하 + 폭발, `meteor_shower` 패턴 기반 보라색/어둠 테마
  - 서버: 각 영웅 위치에 개별 이펙트 push, 클라이언트: `dark_meteor_fall` SkillEffect 렌더링
- **멀티플레이 연결 해제 처리 개선**
  - 팀원 이탈/방장 위임 인게임 알림 추가
  - 연결 해제된 플레이어 캐릭터가 서버 게임 엔진에서 즉시 제거

### V1.24.1
- **난이도별 랭킹 시스템 확장**: 극한/지옥/종말 3개 난이도 독립 랭킹
  - 상단 난이도 탭 3개: 극한(빨강), 지옥(주황), 종말(보라)
  - 하단 인원수 탭 유지 (1/2/3/4인)
  - `difficulty_rankings` 통합 테이블 신규, 기존 `extreme_rankings` 데이터 자동 마이그레이션
  - 서버 API: `GET /:difficulty/:playerCount`, `POST /` 추가 (기존 `/extreme` 하위호환 유지)
  - 상위 10위 표시
...
### V1.19.4
- **난이도별 랭킹 시스템**: 클리어 기록 저장 및 조회
  - 극한/지옥/종말 난이도별 독립 랭킹 (V1.24.1에서 확장)
  - 1인/2인/3인/4인 플레이어 수별 별도 랭킹
  - 로비 화면에 🏆 랭킹 버튼 추가
  - 극한/지옥/종말 난이도 승리 시 자동 저장
