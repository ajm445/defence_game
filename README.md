...
- **이동 텔레포트 버그 수정 + 배포 환경 이동 둔화 수정 (위치 동기화 개선)**
  ...
  - 보정 크기 상한(cap) 도입: 이동 중 8px/프레임, 정지 중 5px/프레임 (큰 차이에서도 부드러운 수렴)
  - 스냅 임계치: 200px → 400px (부활/초기 생성 등 극단 상황에만 적용)
  - 데드존 기반 보정: 이동 중 25px, 정지 중 20px 이하 차이 무시 (네트워크 지연 슬라이딩 방지)
  - 정지 시 보정 완화: lerp 20%, cap 5px (방향 전환/정지 후 슬라이딩 제거)
  - 서버 위치 보정: 10~200px에서 20% 보간 (지연 환경에서 stale 위치 역방향 보정 완화)
  - 지속 이동 중 주기적 위치 보고 (1초 간격) 추가로 드리프트 누적 방지
