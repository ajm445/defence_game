...
### V1.24.6 (현재)
- **RPG 맵 UI 대폭 개선**
  - 지형 장식 추가: 풀 패치(150개), 바위(40개), 물웅덩이(12개, 반짝임 애니메이션), 횃불(16개, 불꽃 흔들림 + 빛 글로우)
  - 맵 경계 자연화: 금색 점선 제거 → 나무/바위 벽으로 교체 (2열 배치)
  - 경계 밖 어둠 처리: 맵 안쪽 250px부터 점진적 그라데이션 → 경계 밖 완전 검정
  - 영역별 색조: 넥서스 주변 시안(안전 영역), 적 기지 주변 적갈색(위험 영역)
  - 안개/파티클 효과: 가장자리 안개, 넥서스 빛 파티클, 기지 주변 붉은 연기
  - 미니맵 개선: 영역 색조 반영, 어두운 초록 배경, 테두리 색상 변경
  - 시드 기반 랜덤 생성으로 매 게임 동일 배치, 기지/넥서스 근처 충돌 방지
  - 신규 파일: `drawMapDecorations.ts` (장식/경계/색조/파티클 통합 렌더러)
