# 게임 밸런스

> 게임 수치, AI 행동 패턴, 밸런스 분석

---

## 목차

1. [핵심 수치](#핵심-수치)
2. [AI 시스템](#ai-시스템)
3. [전투 밸런스](#전투-밸런스)
4. [경제 밸런스](#경제-밸런스)
5. [승률 분석](#승률-분석)

---

## 핵심 수치

### 게임 설정

| 항목 | 플레이어 | AI (난이도별) | 설명 |
|------|---------|--------------|------|
| 게임 시간 | 600초 (10분) | - | 제한 시간 |
| 맵 너비 | 3000px | - | 좌우 크기 |
| 맵 높이 | 2000px | - | 상하 크기 |
| 골드 수입 | 4/초 | 3~7/초 | 난이도별 상이 |
| 기지 HP | 1000 | 1000~1500 | 난이도별 상이 |

### 난이도별 AI 설정

| 항목 | 쉬움 | 중간 | 어려움 |
|------|------|------|--------|
| 골드 수입 | 3/초 | 5/초 | 7/초 |
| 행동 주기 | 5초 | 3초 | 2초 |
| 행동 확률 | 60% | 80% | 95% |
| 초기 골드 | 100 | 150 | 200 |
| 적 기지 HP | 1000 | 1200 | 1500 |

### UI 설정

| 항목 | 값 |
|------|-----|
| UI 패널 높이 | 120px |
| 미니맵 너비 | 200px |
| 미니맵 높이 | 150px |

---

## AI 시스템

### AI 의사결정

AI는 **난이도별 행동 주기**마다 행동을 결정합니다.

```
쉬움: 5초 주기, 60% 확률
중간: 3초 주기, 80% 확률
어려움: 2초 주기, 95% 확률
```

### AI 행동 로직

```javascript
// AI 의사결정 순서 (우선순위 순)

0. 약초 판매 (행동 확률 무관)
   - 약초 >= 10
   → 30골드로 판매

1. 행동 여부 결정
   - 난이도별 확률로 행동
   - 나머지 확률로 이번 턴 스킵

2. 지원 유닛 체크
   - 지원 유닛 < 최소 수 AND 골드 >= 30
   → 50% 나무꾼 / 25% 채집꾼 / 25% 광부

3. 금광부 생산 시도
   - 금광부 < 1 AND 자원 충분
   → 확률적으로 금광부 생산

4. 채집꾼 생산 시도
   - 채집꾼 < 2 AND 골드 >= 35
   → 확률적으로 채집꾼 생산

5. 광부 생산 시도
   - 광부 < 2 AND 골드 >= 40 AND 나무 >= 5
   → 확률적으로 광부 생산

6. 기사 생산 시도
   - 자원 충분 (골드+나무+돌)
   → 확률적으로 기사 생산

7. 궁수 생산 시도
   - 나무 >= 10 AND 골드 >= 80
   → 확률적으로 궁수 생산

8. 기본 행동
   - 골드 >= 50
   → 검병 생산
```

### AI 유닛 생산 확률 (난이도별)

| 유닛 | 쉬움 | 중간 | 어려움 |
|------|------|------|--------|
| 금광부 | 20% | 40% | 50% |
| 채집꾼 | 20% | 35% | 45% |
| 광부 | 20% | 35% | 45% |
| 기사 | 25% | 45% | 55% |
| 궁수 | 30% | 50% | 60% |

### AI 강점과 약점

**강점**:
- 지원 유닛을 일정 수 유지 (난이도별 3~5마리)
- 다양한 지원 유닛 생산 (나무꾼, 광부, 채집꾼, 금광부)
- 약초 자동 판매로 추가 골드 확보
- 안정적인 자원 수급
- 지속적인 유닛 생산

**약점**:
- 벽/업그레이드 미사용
- 공격적 전략 부재
- 수동적인 방어 플레이

---

## 전투 밸런스

### 유닛 대 유닛

#### 검병 vs 검병

```
검병 HP: 100
검병 공격력: 15
쿨다운: 1초

1:1 전투 시간 = 100 / 15 = 6.67초
→ 선공 유닛 승리 (남은 HP: 약 15)
```

#### 궁수 vs 궁수

```
궁수 HP: 50
궁수 공격력: 25
쿨다운: 1초

1:1 전투 시간 = 50 / 25 = 2초
→ 선공 유닛 승리 (남은 HP: 25)
```

#### 검병 vs 궁수

```
검병이 궁수에게 접근하는 동안 피해:
- 궁수 사거리: 150px
- 검병 속도: 2px/frame

접근 시간 ≈ 75 프레임 ≈ 1.25초
접근 중 받는 피해 ≈ 25~50 (1~2회 공격)

결과: 검병 HP 50~75로 도착
→ 검병 승리 (단, 피해 큼)
```

### 유닛 대 기지

| 유닛 | DPS | 기지 파괴 시간 (단독) |
|------|-----|---------------------|
| 검병 | 15 | 66.7초 |
| 궁수 | 25 | 40초 |

### 효율적인 조합

| 조합 | 총 비용 | 총 DPS | 비용당 DPS |
|------|---------|--------|----------|
| 검병 5명 | 250G | 75 | 0.30 |
| 궁수 3명 | 240G + 30W | 75 | 0.28* |
| 검병 3 + 궁수 2 | 310G + 20W | 95 | 0.29* |

*나무 비용 미포함 시

---

## 경제 밸런스

### 자동 수입 분석

```
플레이어 골드 자동 수입: 4/초
AI 골드 자동 수입 (난이도별):
  - 쉬움: 3/초
  - 중간: 5/초
  - 어려움: 7/초

10분 총 자동 수입:
  - 플레이어: 2,400 골드
  - AI (쉬움): 1,800 골드
  - AI (중간): 3,000 골드
  - AI (어려움): 4,200 골드

시작 자원:
  - 플레이어: 100 골드
  - AI (쉬움): 100 골드
  - AI (중간): 150 골드
  - AI (어려움): 200 골드
```

### 유닛 생산 가능량

| 유닛 | 비용 | 최대 생산 가능 |
|------|------|---------------|
| 검병 | 50G | 50마리 |
| 궁수 | 80G | 31마리 (나무 제한) |
| 나무꾼 | 30G | 83마리 |

### 이상적인 경제 운영

```
초반 5분 (600초):
- 자동 수입: 1,200G
- 권장 지출:
  - 나무꾼 2명: 60G
  - 광부 1명: 40G + 5W
  - 검병 3명: 150G
  - 잔액: ~950G + 지원 유닛 채집량
```

### 손익분기점 분석

| 유닛 | 비용 | 채집 속도 | 손익분기점 |
|------|------|----------|-----------|
| 나무꾼 | 30G | 1.0/초 | 30초 |
| 광부 | 40G + 5W | 0.8/초 | 50초 |
| 채집꾼 | 35G | 1.2/초 | 29초 |

---

## 승률 분석

### 승리 조건별 확률

| 승리 조건 | 난이도 | 권장 전략 |
|----------|--------|----------|
| 적 기지 파괴 | 중 | 공격 유닛 집중 |
| 시간 종료 HP 우위 | 쉬움 | 방어 + 업그레이드 |

### 전략별 예상 승률

| 전략 | 설명 | 예상 승률 |
|------|------|----------|
| 러시 | 초반 검병 대량 생산 | 40% |
| 균형 | 자원 + 유닛 균형 | 60% |
| 거북이 | 방어 후 역습 | 55% |
| 궁수 스팸 | 중반 이후 궁수 집중 | 65% |

### 핵심 승리 요소

1. **자원 수급** - 지원 유닛 조기 고용
2. **유닛 효율** - 검병 + 궁수 조합
3. **시간 관리** - 후반 공세 타이밍
4. **HP 관리** - 벽과 업그레이드 활용

---

## 밸런스 요약

### 현재 밸런스 상태

| 요소 | 평가 | 비고 |
|------|------|------|
| 자원 시스템 | 난이도별 조절 | 쉬움은 유리, 어려움은 불리 |
| 유닛 밸런스 | 균형 | 기사(HP 250, 공격력 30) 포함 다양화 |
| AI 난이도 | 3단계 선택 | 쉬움/중간/어려움 |
| 적 기지 HP | 난이도별 | 1000/1200/1500 |
| 게임 시간 | 적절 | 10분 내 결판 |

### 난이도별 밸런스

| 난이도 | 플레이어 우위 | 주요 요소 |
|--------|-------------|----------|
| 쉬움 | 매우 유리 | 자원 25% 우위, 동일 기지 HP |
| 중간 | 불리 | 자원 25% 열세, 기지 HP +20% |
| 어려움 | 매우 불리 | 자원 75% 열세, 기지 HP +50% |

### 추천 개선 사항

1. 수정 활용처 추가
2. 추가 유닛 타입 도입
3. AI 벽 건설/업그레이드 로직
4. 난이도별 AI 전략 다양화

---

## 다음 문서

- [게임플레이 가이드](./GAMEPLAY.md) - 기본 조작 및 전략
- [유닛 가이드](./UNITS.md) - 모든 유닛의 상세 정보
- [자원 시스템](./RESOURCES.md) - 자원 획득 및 관리
